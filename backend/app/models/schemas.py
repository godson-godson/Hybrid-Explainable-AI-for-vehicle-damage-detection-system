...
class LLMSurveyReport(BaseModel):
    ...
    surveyor_recommendation: str = Field("Detailed Teardown Audit Required", description="Assistive surveyor audit recommendation: e.g., Detailed Teardown Audit Required, Physical Verification Recommended")
    claim_disposition: str = Field("Detailed Teardown Audit Required", description="Advisory verdict/recommendation retained for backwards compatibility")
    total_estimated_labor_hours: Optional[float] = Field(None, description="Total estimated teardown/inspection labor hours aggregated from Knowledge Graph")
    estimated_repair_cost_min: Optional[float] = Field(None, description="Estimated minimum repair expense in INR (if pricing data available)")
    estimated_repair_cost_max: Optional[float] = Field(None, description="Estimated maximum repair expense in INR (if pricing data available)")
